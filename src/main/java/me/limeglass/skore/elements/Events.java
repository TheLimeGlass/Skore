...
public class Events {
	...
	public static void registerEvent(@Nullable Class<? extends SkriptEvent> skriptEvent, Class<? extends Event> event, String... patterns) {
		if (!Skore.getInstance().getConfig().getBoolean("Events", true)) return;
		...
		for (int i = 0; i < nodes.length; i++) {
			if (!Skore.getInstance().getConfiguration("syntax").isSet("Syntax.Events." + event.getSimpleName() + "." + nodes[i])) {
