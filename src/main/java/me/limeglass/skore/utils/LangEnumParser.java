...
import me.limeglass.skore.lang.SkoreParser;

public class LangEnumParser<T extends Enum<T>> extends SkoreParser<T> {
	...
	public LangEnumParser(String variableNamePattern, Class<T> clazz) {
		...
		try {
			...
			Skore.getInstance().getConfiguration("syntax").set("Syntax.Enums." + clazz.getSimpleName() + ".names", enumNames);
		} catch (NullPointerException error) {
			Skore.consoleMessage("&cThe class: " + clazz.getName() +  " for classinfo name: " + variableNamePattern + " isn't an Enum!");
		}
		Skore.save("syntax");
	}
