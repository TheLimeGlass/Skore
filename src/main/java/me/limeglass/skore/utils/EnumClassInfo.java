...
public class EnumClassInfo<E extends Enum<E>> {
	...
	public void register() {
		if (Classes.getExactClassInfo(enumType) == null) {
			if (Skore.getInstance().getConfig().getBoolean("UseEnglishEnums")) {
				...
				Skore.getInstance().getConfiguration("syntax").set("Syntax.Enums." + enumType.getSimpleName() + ".enums", enums);
			}
	...
	private void enumDebug(String message, String codeName, Class<?> clazz) {
		if (codeName.equalsIgnoreCase(Skore.getInstance().getConfig().getString("syntaxDebug"))) Skore.debugMessage("&eRegistered syntaxDebug found enum classinfo: " + codeName + " (" + clazz.getCanonicalName() + ")");
		else Skore.debugMessage("&5Registered Enum '" + codeName + "' with return class " + enumType.getName());
		Skore.getInstance().getConfiguration("syntax").set("Syntax.Enums." + clazz.getSimpleName() + ".user", user);
		Skore.save("syntax");
	}
